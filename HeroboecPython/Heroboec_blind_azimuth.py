...
class HeroboecBlindAzimuthScenario:
    ...
    def _load_config(self, path: str):
        ...
        self.roll_control_factor = float(cfg.get("roll_control_factor", 3.0))
        ...
        self.mode_angle_value = float(cfg.get("mode_angle_value", 0.0))
        
        print(f"Config c_t={self.channel_throttle} c_a={self.channel_arm}")
    ...
    def stop_all(self):
        ...
        self.controller.set_channel(self.channel_roll, 0)    # Roll
        self.controller.set_channel(self.channel_pitch, 0)    # Pitch
        self.controller.set_channel(self.channel_throttle, -1)   # Throttle
        self.controller.set_channel(self.channel_yaw, 0)    # Yaw
        time.sleep(0.5)
        self.controller.set_channel(self.channel_arm, 0)    # Disarm
        time.sleep(0.5)
    ...
    def _apply_attitude(self):
        ...
        self.controller.set_channel(self.channel_roll, roll)
        self.controller.set_channel(self.channel_pitch, pitch)
        self.controller.set_channel(self.channel_yaw, yaw)
    ...
    def _azimuth_loop(self):
        ...
        while self.running and self.azimuth_loop_running:
            ...
            roll = self.lerp(self.roll_control_min, self.roll_control_max, (control * self.roll_control_factor + 1.0) * 0.5)
    ...
    def _height_loop(self):
        ...
        while self.running and self.height_loop_running:
            ...
            #current_alt = tlm.gps.altitude - self.zero_altitude
            #current_alt = self.zero_altitude + self.target_height - 50
            current_alt = tlm.v_speed.altitude - self.zero_altitude
            desired_alt = self.target_height
            ...
            log_print(f"H={current_alt:.1f} tH={desired_alt:.1f} iH={tlm.v_speed.altitude:.1f} e={error:.3f} c={control:.3f} cp={pitch:.3f} gA={tlm.gps.altitude:.1f}")
    ...
    def start_scenario_sync(self):
        ...
        log_print(f"Channels: roll={self.channel_roll} pitch={self.channel_pitch} yaw={self.channel_yaw} throttle={self.channel_throttle} arm={self.channel_arm}")
        self.controller.set_channel(self.channel_roll, 0)    # Roll
        self.controller.set_channel(self.channel_pitch, 0)    # Pitch
        self.controller.set_channel(self.channel_throttle, -1)   # Throttle (минимум)
        self.controller.set_channel(self.channel_yaw, 0)    # Yaw
        self.controller.set_channel(self.channel_arm, 1)    # Arm
        self.controller.set_channel(self.channel_mode, self.mode_acro_value)    # Mode ACRO (как в исходном сценарии)
        ...
        tlm = self.controller.get_telemetry()
        self.zero_altitude = tlm.v_speed.altitude
        ...
        success = self.tween_alpha(
            ...
            lambda a: self.controller.set_channel(self.channel_throttle, self.lerp(-1.0, self.half_throttle, a))
        )
        if not success:
            return
        ...
        success = self.tween_alpha(
            ...
            lambda a: self.controller.set_channel(self.channel_throttle, self.lerp(self.half_throttle, self.max_throttle, a))
        )
        if not success:
            return
        ...
        self.controller.set_channel(self.channel_mode, self.mode_angle_value)   # Mode ANGLE (предполагается, что 0 соответствует Angle)
        with self._attitude_lock:
        ...
        self.controller.set_channel(self.channel_throttle, -1)
