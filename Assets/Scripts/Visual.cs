...
public class Visual : MonoBehaviour{
    ...
    void Update(){
        ...
        // Вычисляем смещение камеры относительно начальной позиции
        Vector3 offset = currentCameraPosition - initialCameraPosition;

        // Преобразуем смещение в локальные координаты камеры с учетом её поворота
        Vector3 localOffset = Camera.main.transform.InverseTransformDirection(offset);

        // Используем локальные координаты x и z для установки позиции UI Image
        Vector2 newPosition = new Vector2(localOffset.x, localOffset.y);

        // Устанавливаем позицию UI Image
        centerp.anchoredPosition = newPosition * -1000f; // Умножаем на -0.1f, как в вашем коде
